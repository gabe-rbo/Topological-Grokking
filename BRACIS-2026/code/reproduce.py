...
def main():
    ...
    parser.add_argument("--backend", type=str, default="auto", choices=["auto", "cpu", "cuml", "mlx"],
                         help="UMAP backend, forwarded to run_analysis.py (default: auto -- CUDA/cuml if "
                              "available, else Apple Silicon/mlx, else cpu). Only (umap, cpu) is verified "
                              "bit-for-bit reproducible; the default trades that guarantee for speed -- pass "
                              "--backend cpu for an exactly-reproducible run.")
