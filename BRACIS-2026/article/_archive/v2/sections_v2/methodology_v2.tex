...
We apply UMAP~\cite{McInnes2018} to reduce $A^{(\ell)}_t$ to $\tilde{A}^{(\ell)}_t \in \mathbb{R}^{n \times 2\hat{d}}$, with target dimension $2\hat{d}$ motivated by the Strong Whitney Embedding Theorem (Section~\ref{subsec:id}). As noted in Section~\ref{subsec:id}, UMAP provides no formal topological guarantees; the reported Betti numbers should be interpreted as indicative of structural trends instead exact invariants.
